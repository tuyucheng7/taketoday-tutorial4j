## 相关文章

+ [Ehcache简介](docs/Ehcache简介.md)
+ [Spring缓存指南](docs/Spring缓存指南.md)
+ [Spring Cache：创建一个自定义的KeyGenerator](docs/SpringCache-创建一个自定义的KeyGenerator.md)
+ [Spring Boot中的缓存逐出](docs/SpringBoot中的缓存逐出.md)
+ [在Spring中使用多个缓存管理器](docs/在Spring中使用多个缓存管理器.md)
+ [测试Repository上的@Cacheable](docs/测试Repository上的@Cacheable.md)
+ [Spring Boot Ehcache示例](docs/SpringBoot-Ehcache示例.md)

- 更多文章： [[next -->]](../spring-caching-2/README.md)