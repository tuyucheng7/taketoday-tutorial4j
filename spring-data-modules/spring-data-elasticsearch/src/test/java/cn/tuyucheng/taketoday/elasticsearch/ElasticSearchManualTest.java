...
public class ElasticSearchManualTest {
   ...
   public void givenMultipleQueries_thenReturnResults() throws Exception {
      ...
      response1.hits().hits().forEach(hit -> LOGGER.info("Response 1: {}", hit.source()));
      ...
      response2.hits().hits().forEach(hit -> LOGGER.info("Response 2: {}", hit.source()));
      ...
      response3.hits().hits().forEach(hit -> LOGGER.info("Response 3: {}", hit.source()));
      ...
      response4.hits().hits().forEach(hit -> LOGGER.info("Response 4: {}", hit.source()));

      SearchResponse<Person> response5 = client.search(s -> s.query(q -> q.bool(b -> b
            .must(ageQuery)
            .must(fullNameQuery)
            .must(simpleStringQuery))), Person.class);
      response5.hits().hits().forEach(hit -> LOGGER.info("Response 5: {}", hit.source()));
   }
}