...
class ArchaiusJDBCSourceLiveTest {
	...
	private static Map<String, String> createExpectedArchaiusProperties() {
		...
		map.put("tuyucheng.archaius.properties.one", "one FROM:jdbc_source");
		map.put("tuyucheng.archaius.properties.two", "two FROM:application.properties");
		map.put("tuyucheng.archaius.properties.three", "three FROM:jdbc_source");
		return map;
	}
