...
class TodosControllerApiIntegrationTest {
	...
	void testFindAllTodos() throws Exception {
		...
		mvc.perform(get(BASEURL).accept(DEFAULT_MEDIA_TYPE))
	...
	void testFindById() throws Exception {
		...
		mvc.perform(get(BASEURL + "/1").accept(DEFAULT_MEDIA_TYPE))
			.andExpect(status().isOk())
			.andExpect(content().contentType(DEFAULT_MEDIA_TYPE))
	...
	void testCreateTodo() throws Exception {
		...
		mvc.perform(post(BASEURL).contentType(DEFAULT_MEDIA_TYPE).content(json))
	...
	void testUpdateTodo() throws Exception {
		final var json = "{\"title\":\"test-todo\"}";
		mvc.perform(put(BASEURL + "/5").contentType(DEFAULT_MEDIA_TYPE).content(json))
	...
	void testDeleteTodo() throws Exception {
		mvc.perform(delete(BASEURL + "/5").contentType(DEFAULT_MEDIA_TYPE))
			.andExpect(status().isNoContent());
	}
	...
	void whenThereIsNoSlashMatching_ThenHttpStatusIs404() throws Exception {
		mvc.perform(get(BASEURL + "/name/").contentType(DEFAULT_MEDIA_TYPE))
			.andExpect(status().isNotFound());
	}
