...
我们可以跳过创建表并简单地依赖spring-boot-starter-data-cassandra为我们初始化schema，
...
加载应用程序上下文时，框架将使用派生自它的父类匹配每个Repository类型：
...
使用CrudRepository触发BookDocument保存仍然成功，但第6行返回的类型现在是Iterable而不是List：
