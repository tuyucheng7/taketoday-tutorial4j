...
在这个教程中，**我们将重点介绍Spring为JPA Repository提供的不同类型的BootstrapMode，用于改变其实例化的编排**。
...
随着Repository数量的增加，应用程序可能需要很长时间才能开始接受请求。
...
在我们的示例中，**我们尽早地初始化Repository，并在应用程序启动后使其可用**。

### 2.2 Lazy

通过对JPA Repository使用惰性BootstrapMode，Spring注册了Repository的bean定义，但不会立即实例化它。因此，使用LAZY选项，只会在第一次使用触发其初始化。
...
**我们应该只在开发过程中使用LAZY模式，以避免在生产环境中部署应用程序时出现潜在的初始化错误**。
...
接下来，我们将EntityManagerFactory bean添加到我们的配置中，并指示我们想要使用异步执行器进行后台启动：
...
简而言之，Spring将Repository标记为惰性，并注册一个DeferredRepositoryInitializationListener bean。
当ApplicationContext触发ContextRefreshedEvent事件时，它会初始化所有Repository。
