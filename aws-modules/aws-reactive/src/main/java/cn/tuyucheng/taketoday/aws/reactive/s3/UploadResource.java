package cn.tuyucheng.taketoday.aws.reactive.s3;
...
public class UploadResource {
	...
	public Mono<ResponseEntity<UploadResult>> uploadHandler(@RequestHeader HttpHeaders headers, @RequestBody Flux<ByteBuffer> body) {
		...
		LOGGER.info("[I95] uploadHandler: mediaType{}, length={}", mediaType, length);
	...
	protected Mono<String> saveFile(HttpHeaders headers, String bucket, FilePart part) {
		...
		LOGGER.info("[I137] saveFile: filekey={}, filename={}", filekey, part.filename());
		...
		return Mono
			...
			.flatMapMany((response) -> {
				...
				LOGGER.info("[I183] uploadId={}", response.uploadId());
				return part.content();
			})
			.bufferUntil((buffer) -> {
				uploadState.buffered += buffer.readableByteCount();
				if (uploadState.buffered >= s3config.getMultipartMinPartSize()) {
					LOGGER.info("[I173] bufferUntil: returning true, bufferedBytes={}, partCounter={}, uploadId={}", uploadState.buffered, uploadState.partCounter, uploadState.uploadId);
			...
			.reduce(uploadState, (state, completedPart) -> {
				LOGGER.info("[I188] completed: partNumber={}, etag={}", completedPart.partNumber(), completedPart.eTag());
	...
	private static ByteBuffer concatBuffers(List<DataBuffer> buffers) {
		LOGGER.info("[I198] creating BytBuffer from {} chunks", buffers.size());
		...
		LOGGER.info("[I208] partData: size={}", partData.capacity());
		return partData;
	...
	private Mono<CompletedPart> uploadPart(UploadState uploadState, ByteBuffer buffer) {
		...
		LOGGER.info("[I218] uploadPart: partNumber={}, contentLength={}", partNumber, buffer.capacity());
		...
		return Mono
			...
			.map((uploadPartResult) -> {
				...
				LOGGER.info("[I230] uploadPart complete: part={}, etag={}", partNumber, uploadPartResult.eTag());
	...
	private Mono<CompleteMultipartUploadResponse> completeUpload(UploadState state) {
		LOGGER.info("[I202] completeUpload: bucket={}, filekey={}, completedParts.size={}", state.bucket, state.filekey, state.completedParts.size());
