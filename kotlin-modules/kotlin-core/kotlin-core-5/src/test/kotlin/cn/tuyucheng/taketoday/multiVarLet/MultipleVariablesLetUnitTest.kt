package cn.tuyucheng.taketoday.multiVarLet
	...
	fun `given two nullable variables when calling let2 on them should get expected result`() {
		assertThat(
			let2(
				"Kai",
				7
			) { name, num -> "Hi $name, $num squared is ${num * num}" }).isEqualTo("Hi Kai, 7 squared is 49")
	...
	fun `given three nullable variables when calling whenAllNotNullLet on them should get expected result`() {
		assertThat(
			letIfAllNotNull(
				...
				7,
				8
			) { "${it.joinToString(separator = " + ") { num -> "$num" }} is ${it.sum()}" }).isEqualTo("5 + 6 + 7 + 8 is 26")
		assertThat(
			letIfAllNotNull(
				5,
				6,
				7
			) { "${it.joinToString(separator = " + ") { num -> "$num" }} is ${it.sum()}" }).isEqualTo("5 + 6 + 7 is 18")
		assertThat(
			letIfAllNotNull(
				5,
				7
			) { "${it.joinToString(separator = " + ") { num -> "$num" }} is ${it.sum()}" }).isEqualTo("5 + 7 is 12")
		...
		assertThat(
			letIfAllNotNull(
				...
				7,
				6
			) { "${it.joinToString(separator = " + ") { num -> "$num" }} is ${it.sum()}" }).isNull()
		assertThat(
			letIfAllNotNull(
				nullNum,
				null,
	...
	fun `given three nullable variables when calling whenAnyNotNullLet on them should get expected result`() {
		...
		assertThat(
			letIfAnyNotNull(
				...
				6,
				7
			) { "${it.joinToString(separator = " + ") { num -> "$num" }} is ${it.sum()}" }).isEqualTo("6 + 7 is 13")
		assertThat(
			letIfAnyNotNull(
				nullNum,
				nullNum,
				7
			) { "${it.joinToString(separator = " + ") { num -> "$num" }} is ${it.sum()}" }).isEqualTo("7 is 7")
		assertThat(
			letIfAnyNotNull(
				nullNum,
				nullNum,
