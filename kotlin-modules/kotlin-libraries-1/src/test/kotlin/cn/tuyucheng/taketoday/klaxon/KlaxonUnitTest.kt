...
class KlaxonUnitTest {
	...
	fun giveJsonString_whenParser_thenGetJsonObject() {
		...
		assertThat(json).hasSize(3).containsEntry("name", "HDD").containsEntry("capacityInGb", 512)
			...
			reader.beginObject {
				while (reader.hasNext()) {
					when (val readName = reader.nextName()) {
						...
						"features" -> assertThat(reader.nextObject()).containsEntry("cacheInMb", 64)
							.containsEntry("speedInRpm", 7200)
