...
**网站通常会在用户已经登录时阻止他们访问login**。
一种常见的方法是将用户重定向到另一个页面，通常是登录后应用程序的首页。
...
**换句话说，我们需要从SecurityContext中获取Authentication详细信息并验证用户是否已登录**：
...
拦截器会在请求到达控制器之前拦截它。因此，我们可以根据authentication来决定是让它继续进行还是阻止它并返回重定向响应。

如果用户通过了身份验证，我们需要在响应中修改两件事：

+ 将状态码设置为HttpStatus.SC_TEMPORARY_REDIRECT。
+ 添加带有重定向URL的Location header。
...
可以使用Spring Security的过滤器链将过滤器直接应用于SecurityContext。因此，它可以在创建authentication后立即拦截请求。

让我们继承GenericFilterBean，重写doFilter方法，并验证authentication：
...
此外，我们需要对登录页面URI的请求进行授权，以便为其启用过滤器链：
