...
在一个典型的基于抽象的设计中，一个接口有一个或多个实现，如果一个或多个方法被添加到接口中，所有的实现都将被迫实现它们。否则，设计就会崩溃。
...
在这种情况下，代码根本无法编译，因为存在由多个接口继承引起的冲突(也称为[钻石问题](https://en.wikipedia.org/wiki/Multiple_inheritance))。Car类将继承两组默认方法。那么我们应该调用哪些？
