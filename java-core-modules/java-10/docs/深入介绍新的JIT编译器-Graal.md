...
这意味着我们可以通过三种不同的方式运行一个简单的程序：使用常规的分层编译器、使用Java 10上的JVMCI版本的 Graal、或者使用GraalVM本身。
...
现在让我们介绍另一种类型的边，即描述控制流的边。为此，我们将通过调用方法来检索我们的变量而不是直接读取它们来扩展我们的示例。当我们这样做时，我们需要跟踪方法调用顺序，我们用红色箭头表示这个顺序：
